...
export default function SupervisorPage() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(([dbPeople, dbAccessUsers, auth]) => {
        ...
        const access = users.find((user) => user.email.toLowerCase() === email) ?? null;
  ...
  const effectiveGroup = supervisorGroup ?? groupFilter;
    ...
    () =>
      people.filter((person) => {
        ...
        const belongsToGroup =
          effectiveGroup === "todos" || person.grupo === effectiveGroup;
  ...
  const supervisorLabel = currentAccess
    ? `${currentAccess.nombre} | ${
        currentAccess.rol === "supervisor"
          ? formatGroup(currentAccess.grupo ?? "grupo-1")
          : "Administrador"
      }`
    : "Sin acceso asociado";
      ...
      <Paper elevation={0} sx={{ p: 2, borderRadius: 2 }}>
        ...
        >
          ...
          <TextField
            ...
            disabled={Boolean(supervisorGroup)}

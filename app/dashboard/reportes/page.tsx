...
export default function Page() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      .then(
        ...
        const access =
          dbAccessUsers?.find((user) => user.email.toLowerCase() === email) ?? null;
        ...
        if (access?.rol === "supervisor" && access.grupo) {
          setFilters((current) => ({ ...current, grupo: access.grupo as GroupFilter }));
        }
      })
      .finally(() => {
        if (active) {
  ...
  const visibleOpenRuns = useMemo(() => {
    ...
    return openRuns
      ...
      .filter((run) => selectedGroup === "todos" || !selectedGroup || run.grupo === selectedGroup);
    ...
    <Stack spacing={3}>
      <Box>
        ...
        <Typography color="text.secondary">
          Historico por fecha, grupo de personal, turno, desempeno por celda y equipos de lineas.
        </Typography>
      </Box>
          ...
          <Box sx={{ overflowX: "auto" }}>
            ...
            <TableHead>
              <TableRow>
                ...
                <TableCell>Codigo equipo</TableCell>
      ...
      <PerformanceTable
        title="Desempeno por celda"
        rows={report.desempenoPorCelda}
        firstHeader="Celda"
      />
      <PerformanceTable
        title="Desempeno por equipo de lineas"
  ...
  return (
    <Paper elevation={0} sx={{ p: 2, borderRadius: 2 }}>
      ...
      >
        <Box>
          <Typography variant="h6" sx={{ fontWeight: 800 }}>
            Grafica de cumplimiento por recorrido
          </Typography>
          <Typography color="text.secondary">
    ...
    <Paper elevation={0} sx={{ p: 2, borderRadius: 2 }}>
      ...
      {!!rows.length && (
        <Table size="small">
          <TableHead>
            <TableRow>
              <TableCell>Equipo / lineas</TableCell>
              <TableCell>Recorridos</TableCell>

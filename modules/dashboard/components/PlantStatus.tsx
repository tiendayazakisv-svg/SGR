...
export default function PlantStatus() {
  ...
  const [visibleGroup, setVisibleGroup] = useState<"todos" | SupplyCrewGroup>("todos");
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setAssignments((dbAssignments ?? []).filter((assignment) => !assignment.vigenteHasta));
      setPeople(dbPeople ?? []);
      setLineGroups(catalog?.lineGroups ?? []);
      setVisibleGroup(
  ...
  const visibleAssignments = useMemo(
    () =>
      assignments.filter((assignment) => {
        ...
        return visibleGroup === "todos" || person?.grupo === visibleGroup;
      }),
  ...
  return (
    <Paper sx={{ p: 3 }}>
      ...
      <Typography variant="subtitle2" sx={{ mb: 2 }} color="text.secondary">
        Cobertura por equipo de lineas asignado y estado del almacenista.
      </Typography>

...
export default function AssignmentManager({ initialAssignments }: Props) {
  ...
  const isSupervisor = currentAccess?.rol === "supervisor";
  const supervisorGroup = isSupervisor ? currentAccess?.grupo : undefined;
  const visibleGroups = useMemo<SupplyCrewGroup[]>(
    () => (supervisorGroup ? [supervisorGroup] : ["grupo-1", "grupo-2"]),
    [supervisorGroup]
  );
  const visiblePeople = useMemo(
    () =>
      supervisorGroup
        ? people.filter((person) => person.grupo === supervisorGroup)
        : people,
    [people, supervisorGroup]
  );
  const visibleAssignments = useMemo(
    () =>
      supervisorGroup
        ? assignments.filter((assignment) =>
            assignmentBelongsToGroup(assignment, people, supervisorGroup)
          )
        : assignments,
    [assignments, people, supervisorGroup]
  );
  ...
  const supervisors = accessUsers.filter(
    (user) =>
      user.rol === "supervisor" &&
      user.activo &&
      (!supervisorGroup || user.grupo === supervisorGroup)
  );
  ...
  const canRotate = !isSupervisor && allOperationalPeople.length > 0;
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      const sapId = auth.data.user?.user_metadata?.sap_id?.toUpperCase();
      const email = auth.data.user?.email?.toLowerCase();
      const access =
        dbAccessUsers?.find(
          (user) =>
            user.id === auth.data.user?.id ||
            user.sapId.toUpperCase() === sapId ||
  ...
  async function handleSaveAssignment(lineasOverride?: string[]) {
    ...
    if (isSupervisor && !supervisorGroup) {
    ...
    if (supervisorGroup && keeper?.grupo !== supervisorGroup) {
      setAssignmentMessage({
        severity: "error",
      ...
      <Paper elevation={0} sx={{ p: 2, borderRadius: 2 }}>
        ...
        {isSupervisor && (
          <Alert severity="info" sx={{ mt: 2 }}>
            Los cambios se guardaran solamente sobre {formatGroupName(supervisorGroup ?? "grupo-1")}.
